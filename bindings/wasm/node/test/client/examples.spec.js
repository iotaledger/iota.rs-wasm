...
const addressOutputs = require("../fixtures/addressOutputs.json");
...
describe.skip('Main examples', () => {
    ...
    it('gets the output of a known output ID', async () => {
        const output = await client.getOutput('0xc1d95ac9c8c0237c6929faf427556c3562055a7155c6d336ee7891691d5525c90100');
        expect(output.metadata.blockId).toBeValidBlockId();
    });
    ...
    it('calculates the balance of an address', () => {
        const testOutputs = addressOutputs;
        ...
        for (const outputResponse of testOutputs) {
            ...
            if ('nativeTokens' in output) {
                output.nativeTokens?.forEach((token) => (totalNativeTokens[token.id] =
        ...
        expect(Object.values(totalNativeTokens).reduce((acc, val) => acc + val)).toBe(200);
    });
    ...
    it('sends a block with a tagged data payload', async () => {
        const blockIdAndBlock = await client.buildAndPostBlock(secretManager, {
            tag: (0, lib_1.utf8ToHex)('Hello'),
            data: (0, lib_1.utf8ToHex)('Tangle'),
        });
